...
void Motion1DPositionVelocityDoubleTimed::configure_position_limits(double _limit_pos_neg, double _limit_pos_pos) {
    configure_position_limits_int(_limit_pos_neg, _limit_pos_pos);
}
void Motion1DPositionVelocityDoubleTimed::configure_velocity_limits(double _limit_vel_neg, double _limit_vel_pos) {
    configure_velocity_limits_int(_limit_vel_neg, _limit_vel_pos);
}

void Motion1DPositionVelocityDoubleTimed::configure_acceleration_limits(double _limit_acc_neg, double _limit_acc_pos) {
    configure_acceleration_limits_int(_limit_acc_neg, _limit_acc_pos);
}

void Motion1DPositionVelocityDoubleTimed::configure_jerk_limits(double _limit_jrk) {
    configure_jerk_limits_int(_limit_jrk);
}

void Motion1DPositionVelocityDoubleTimed::configure_dt(double _dt) {
    configure_dt_int(_dt);
}

void Motion1DPositionVelocityDoubleTimed::reset_state() {
...
bool Motion1DPositionVelocityDoubleTimed::target_has_changed_since_last_run(motion_state_s* target_state, double target_arrival_time) {
...
void Motion1DPositionVelocityDoubleTimed::run_next_timestep() {
...
double Motion1DPositionVelocityDoubleTimed::get_vel_for_pos_mode(motion_state_s* current_state, motion_state_s* target_state, double arrival_time) {
    double vel = (target_state->pos - current_state->pos) / max(dt, arrival_time);
    double vel_limited = minmax(vel, limit_neg.vel, limit_pos.vel);
    return vel_limited;
}

double Motion1DPositionVelocityDoubleTimed::get_time_remaining_for(motion_state_s* current_state, motion_state_s* target_state, double arrival_time) {
...
bool Motion1DPositionVelocityDoubleTimed::check_input_and_config_errors() {
...
bool Motion1DPositionVelocityDoubleTimed::check_output_errors() {

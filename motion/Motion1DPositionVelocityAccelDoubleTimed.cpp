...
void Motion1DPositionVelocityAccelDoubleTimed::configure_position_limits(double _limit_pos_neg, double _limit_pos_pos) {
    configure_position_limits_int(_limit_pos_neg, _limit_pos_pos);
}

void Motion1DPositionVelocityAccelDoubleTimed::configure_velocity_limits(double _limit_vel_neg, double _limit_vel_pos) {
...
void Motion1DPositionVelocityAccelDoubleTimed::configure_acceleration_limits(double _limit_acc_neg, double _limit_acc_pos) {
...
void Motion1DPositionVelocityAccelDoubleTimed::configure_jerk_limits(double _limit_jrk) {
    configure_jerk_limits_int(_limit_jrk);
}

void Motion1DPositionVelocityAccelDoubleTimed::configure_dt(double _dt) {
...
void Motion1DPositionVelocityAccelDoubleTimed::reset_state() {
...
void Motion1DPositionVelocityAccelDoubleTimed::run_next_timestep() {
...
double Motion1DPositionVelocityAccelDoubleTimed::get_accel_for_velocity_mode(motion_state_s* current_state, motion_state_s* target_state, double arrival_time_target, double* time_remaining) {
...
Motion1DInterfaceDouble::motion_state_s Motion1DPositionVelocityAccelDoubleTimed::get_accel_for_position_mode(motion_state_s* current_state, motion_state_s* target_state, double arrival_time_target, double* time_remaining) {
...
void Motion1DPositionVelocityAccelDoubleTimed::update_best_solver(double t[], double a[], double best_t[], double best_a[], int* best_solver, int this_solver) {
    ...
    // first, if the new solver finishes faster, it is assumed to be better.
    // second, if there's a tie but the new solver has a lower abs(id number) it is assumed to be better.
...
bool Motion1DPositionVelocityAccelDoubleTimed::solve_position_with_velocity_limit(motion_state_s* current_state, motion_state_s* target_state, double target_time_remaining, double vmax, double t_out[], double a_out[]) {
    ...
    double t3 = user_arrival_time;
    double v1 = vmax;
    double v2 = vmax;

    double a2 = -(v0*v0 - 2*v0*v1 + v1*v1 + 2*v1*v2 - 2*v1*v3 - v2*v2 + v3*v3) / (2*p0 - 2*p3 + 2*t3*v1);
    double a0 = -a2;
    double t1 = (v0 - v1) / a2;
    double t2 = (v2 - v3 + a2*t3) / a2;
    double p1 = -0.5*a2*t1*t1 + v0*t1 + p0;
    double p2 = p1 - v1*(t1-t2);
...
bool Motion1DPositionVelocityAccelDoubleTimed::solve_position_no_velocity_limit_2_phase(motion_state_s* current_state, motion_state_s* target_state, double target_time_remaining, double vlimit, double t_out[], double a_out[]) {
    ...
    double t2 = target_time_remaining;
    ...
    double x1 = sqrtf(2) * sqrtf(2*p0*p0 - 4*p0*p2 + 2*p0*t2*v0 + 2*p0*t2*v2 + 2*p2*p2 - 2*p2*t2*v0 - 2*p2*t2*v2 + t2*t2*v0*v0 + t2*t2*v2*v2);
    double a0a = -(2*p0 - 2*p2 + x1 + t2*v0 + t2*v2) / (t2*t2);
    double a0b = -(2*p0 - 2*p2 - x1 + t2*v0 + t2*v2) / (t2*t2);
    double a0 = NAN;
    if (!isnan(a0a) && (isnan(a0b) || abs(a0a) > abs(a0b))) a0 = a0a;
    if (!isnan(a0b) && (isnan(a0a) || abs(a0b) > abs(a0a))) a0 = a0b;
    double t1 = (v2 - v0 + a0*t2) / (2*a0);
    double v1 = v0 + a0*t1;
    double a1 = -a0;
    ...
    if (isnan(a0) || isnan(a1) || isnan(t1) || isnan(t2)) {
        valid = false;
        reason += 128;
    }
...
bool Motion1DPositionVelocityAccelDoubleTimed::solve_position_no_velocity_limit_1_phase(motion_state_s* current_state, motion_state_s* target_state, double target_time_remaining, double vlimit, double t_out[], double a_out[]) {
    ...
    double a1_from_ev2 = (v2-v1)/t2;
    double a1_from_ep2 = (-2*(p1-p2+t2*v1))/(t2*t2);
    double a1 = a1_from_ep2;
    ...
    if (fabs(a1_from_ep2 - a1_from_ev2) > 0.0001) {
        valid = false;
        reason += 1;
    }
...
bool Motion1DPositionVelocityAccelDoubleTimed::solve_position_no_velocity_limit_1_phase_drift(motion_state_s* current_state, motion_state_s* target_state, double target_time_remaining, double vlimit, double t_out[], double a_out[]) {
    ...
    double a1 = -(v0*v0 - 2*v0*v2 + v2*v2) / (2*p0 - 2*p2 + 2*t2*v0);
    double t1 = (v0 - v2 + a1*t2) / a1;
    double p1 = p0 + v0*t1 + 0.5*a0*t1*t1;
...
bool Motion1DPositionVelocityAccelDoubleTimed::target_has_changed_since_last_run(motion_state_s* target_state) {
...
void Motion1DPositionVelocityAccelDoubleTimed::determine_target_mode() {
...
//    double position_if_stopped_now = next.pos + next.vel * time_to_make_vel_zero + 0.5 * stopping_acc * powf(time_to_make_vel_zero, 2);
//
...
bool Motion1DPositionVelocityAccelDoubleTimed::check_input_and_config_errors() {
...
bool Motion1DPositionVelocityAccelDoubleTimed::check_output_errors() {
